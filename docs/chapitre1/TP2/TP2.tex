...
\fancyhead[C]{\textbf{TP n°2 : boucles, conditionnelles, fonctions}}
...
{\itshape Plusieurs exercices ont été écrits par mon collègue Pierre Duclosson.}

\begin{introduction}{}
\underline{Objectifs :}

\begin{itemize}[label=\ding{43}]
	\item Prendre en main l'environnement de travail (partages réseaux,  programmation en Python).
	\item Apprivoiser la syntaxe de Python. 
	\item Représenter la trace d'exécution d'un programme avec un tableau d'état.
	\item Contrôler le flot d'exécution avec  des boucles (\texttt{for} ou \texttt{while}), des instructions de branchement (\texttt{if}, \texttt{elif}, \texttt{else}).
	\item Écrire des fonctions en respectant une spécification.
\end{itemize}

\underline{Mode d'emploi : }

\begin{itemize}[label=\ding{43}]
\item On garde le cours du chapitre 1  à portée de main.
\item On recopie ses codes dans les emplacements prévus sur l'énoncé, il faut apprendre à bien indenter le code manuscrit, une partie de l'épreuve terminale se passe sur papier !
\end{itemize}
...
Dans un environnement de développement Python, créer un fichier \texttt{tp2.py} et l'enregistrer dans un dossier pertinent de son espace personnel sur le réseau.

On séparera les codes de chaque exercice dans des \textbf{cellules} qui peuvent exécuter de façon indépendante.

Dans \href{https://docs.spyder-ide.org/current/index.html}{Spyder}, on introduit une cellule avec la séquence de caractères \verb+#%%+.
...
\begin{exercice}{Maîtriser le \texttt{range}}

\begin{enumerate}
	\item Faire afficher les entiers de 10 à 0 de manière décroissante.

	\Pointilles{2}{\linewidth}
		
	\item Faire afficher les entiers pairs compris entre 0 et 50 dans l'ordre croissant.
	
	\Pointilles{2}{\linewidth}
	
	
	\item Faire afficher les entiers pairs compris entre 0 et 50 dans l'ordre décroissants.
...
\begin{exercice}{Boucles imbriquées}
\begin{enumerate}
    \item Quel affichage obtient-on en exécutant le code ci-dessous ?
...
\item Pour les questions suivantes, il faut utiliser obligatoirement deux boucles imbriquées et au maximum deux fois la fonction \verb+print+.

\begin{enumerate}

  
	\item \'Ecrire un script Python qui produit l'affichage 1 ci-dessous. 
	
\Pointilles{4}{\linewidth}

	\item \'Ecrire un script Python qui produit l'affichage 2 ci-dessous. 
...
\begin{exercice}{Tableau d'état}

Compléter le tableau d'états permettant de tracer l'exécution du programme ci-dessous.  Les valeurs des variables sont mesurées après l'exécution de la ligne 5.
...
\hline 
Valeur de \texttt{i} & \ldots & \ldots  & \ldots  & \ldots  & \ldots  & \ldots  \\ 
\hline 
Valeur de \texttt{f} & \ldots & \ldots  & \ldots  & \ldots  & \ldots  & \ldots\\ 
\hline 
Valeur de \texttt{g} & \ldots & \ldots  & \ldots  & \ldots  & \ldots  & \ldots \\ 
\hline 
\end{tabularx} 
\end{center}
...
\begin{exercice}{Conditionnelle avec deux clauses}
Pour transporter des poteaux sur une île, on dispose d'une barge dont la masse utile est de 1800 kg.
\'Ecrire un programme qui demande à l'utilisateur la masse d'un poteau puis le nombre de poteaux et qui affiche le message \og Risque de surcharge !\fg\ si la masse utile est dépassée.
...
\begin{exercice}{Conditionnelle avec plus de deux clauses}
On rappelle que les années bissextiles sont celles qui sont multiples de 4 sauf si elles sont multiples de 100 mais pas de 400. Compléter le code suivant pour qu'il affiche \og Année bissextile \fg\ ou \og Année non bissextile\fg selon la valeur du nombre rentré par l'utilisateur.
...
\begin{exercice}{}
\'Ecrire un programme qui permet de saisir un mot de passe sous la forme d'une cha\^ine de caractères et qui ensuite redemande ce mot de passe jusqu'à ce qu'il soit correct. 
...
\begin{exercice}{}
On appelle logarithme entier d'un nombre réel $x\geqslant 1$, le nombre de fois qu'il faut le diviser par 2 pour obtenir un nombre inférieur à 1.
...
%Sedgewick 1.2.26 p. 54

Écrire une fonction \texttt{jour\_semaine} dont on donne la spécification ci-dessous :
...
On appliquera les formules ci-dessous, valables pour le calendrier grégorien : 
...
Reprendre l'exercice 5 en écrivant une fonction \texttt{bissextile(a)} déterminant si une année est bissextile. On donne ci-dessous la signature de cette fonction c'est-à-dire l'en-tête avec les types attendus pour les paramètres et la valeur de retour.
...
Travaux pratiques pages 40 et 41 du \href{https://mesmanuels.fr/acces-libre/3813624}{manuel Hachette}.
\end{exercice}
