...
Pour déclarer une fonction, on définit son \textbf{en-tête} avec  son \textbf{nom} et des \textbf{paramètres formels} d'entrée. Vient ensuite le bloc d'instructions,  décalé d'une indentation et qui constitue le \textbf{le corps} de la fonction.
...
\begin{itemize}
 ...
 \item la nature du traitement effectué.
\end{itemize}

En plaçant cette \textit{docstring} juste après la signature de la fonction, elle sera accessible à travers l'attibut \verb+__doc__+ ou la fonction \texttt{help} en mode interactif. Ce mécanisme d'introspection facilite l'appropriation d'une bibliothèque, un exemple avec la documentation de la fonction \texttt{randint} du module \texttt{random} : 
...
Depuis le mode interactif enrichi Ipython, on peut afficher le code source de cette fonction de bibliothèque pour avoir un exemple de \texttt{docstring} :

\begin{lstlisting}[style=rond]    
In [2]: %psource random.randint                                                 
...
\subsection{Mise au point de programme}
...
\begin{itemize}
 \item on peut vérifier au début du corps de la fonction des \textbf{préconditions} sur les paramètres, par exemple ici  \textit{a et b non tous nuls} peut se traduire par l'assertion \texttt{assert a != 0 or b != 0}. On parle de \textbf{programmation défensive}.
...
def pgcd(a, b):
	...
	return a
\end{lstlisting} 
\item on peut définir hors de la fonction des \textbf{postconditions} sur les valeurs renvoyées en fonction des entrées. n peut les rassembler dans un \textbf{jeu de tests}. On parle alors de \textbf{tests unitaires} :
...
\bcdanger{} Pou reprendre une citation célèbre d'\href{https://fr.wikipedia.org/wiki/Edsger_Dijkstra}{Edger Dijsktraa} :
