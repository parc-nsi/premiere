...
%\fancyhead[R]{\href{http://creativecommons.org/licenses/by-nc/4.0/}{\includegraphics[scale=0.8]{/home/frederic/icone-cc4.png}}}
\fancyhead[R]{\href{http://creativecommons.org/licenses/by-nc/4.0/}{\includegraphics[scale=0.8]{icone-cc4.png}}}

\fancyhead[C]{\textbf{Simulation de réseau avec Filius}}
...
{\itshape Ce document est directement inspiré d'une ressource présentée pendant la formation SNT 2019 des enseignants de l'Académie de Lyon et  placée sous licence \href{https://creativecommons.org/licenses/by-nc-sa/4.0/}{CC BY-NC-SA 4.0}, de la ressource en ligne de David Roche sur la plateforme \href{https://pixees.fr/informatiquelycee/n_site/snt_internet_sim1.html}{Pixees} en particulier pour l'exercice 4 et de la documentation du logiciel Filius.}


Le logiciel \href{https://www.lernsoftware-filius.de/}{Filius} est un logiciel de simulation de réseau développé par une université allemande. Une \href{http://www.lernsoftware-filius.de/downloads/Introduction_Filius.pdf}{documentation en Anglais} est disponible.

L'interface du logiciel se présente ainsi :

\begin{center}
\includegraphics[scale=0.5]{images/filius-interface.png}
\end{center}

Il existe deux modes d'utilisation.

\begin{itemize}[label=\ding{43}]
	\item Pour réaliser le circuit, on utilise le mode \textit{conception} en cliquant sur l'icône \includegraphics[scale=0.5]{images/conception.png}. Les éléments disponibles sont disposés sur le bord vertical gauche de la fenêtre et il suffit de les glisser/déposer dans la zone centrale de conception.
	
	\item Pour effectuer une simulation et installer des logiciels sur les éléments du réseau, on utilise le mode \textit{simulation} en cliquant sur l'icône \includegraphics[scale=0.5]{images/simulation.png}.
\end{itemize}

\bcdz Pour visualiser correctement le trajet des données sur le réseau, il faut régler la vitesse sur une petite valeur : 10 \% ou 20 \%.
...
\begin{exerciceB}{Connexion pair à pair  de deux machines}

L'objectif de cet exercice est de relier en réseau deux machines. Une telle  connexion est dite \textit{pair à pair}.

Créer un nouveau projet \textbf{Filius} et l'enregistrer dans un dossier pertinent de son espace personnel sur le réseau pédagogique sous le nom \texttt{exercice1.fls}.

	\begin{enumerate}
			\item Passer en mode \textit{conception} et créer un réseau de deux machines hôtes de type portable reliées par un câble.
			\item Faire un clic droit sur une machine et lui attribuer l'adresse IP \texttt{192.168.1.1} comme ci-dessous, en sélectionnant l'option \textit{Utiliser  l'adresse IP comme nom}.
			Attribuer de même l'adresse IP \texttt{192.168.1.2} à l'autre machine.
			\item Pourrait-on choisir \texttt{192.168.256.2}  comme adresse \href{https://fr.wikipedia.org/wiki/Adresse_IP}{IP} ? On pourra rechercher sur le Web le format d'une adresse \href{https://fr.wikipedia.org/wiki/Adresse_IP}{IP}. 

On désigne désormais les machines par leur adresse IP.
			
		\item Passer en mode \textit{simulation}, faire un clic droit sur la machine \texttt{192.168.1.1}, sélectionner \textit{Afficher le bureau} et installer l'application \textit{ligne de commandes} en la faisant glisser vers la zone des applications installées à gauche comme  ci-dessous.
	


\begin{minipage}[t]{0.45\linewidth}
\begin{center}
			\fbox{Paramétrage de l'adresse IP}
			
			\bigskip
			
			\includegraphics[scale=0.35]{images/parametrageIP.png}
			\end{center}
\end{minipage}\hfill
\begin{minipage}[t]{0.45\linewidth}
\begin{center}
			\fbox{Installation d'une application}
			
			\bigskip
			
			\includegraphics[scale=0.35]{images/installation-ping2.png}
			\end{center}
\end{minipage}


   \item Lancer l'application ligne de commandes sur la machine \texttt{192.168.1.1} puis exécuter la commande \texttt{ping 192.168.1.2} pour tester la connexion vers la machine \texttt{ping 192.168.1.2}. Le câble devrait se colorer en vert si la connexion est correcte et les quatre paquets de données envoyés par \texttt{ping} devraient recevoir un écho \texttt{pong} retourné par \texttt{ping 192.168.1.2}.
   

\begin{minipage}[t]{0.4\linewidth}
\begin{center}
			\fbox{\texttt{ping 192.168.1.2}}
			
			\bigskip
			
			\includegraphics[scale=0.35]{images/exo1-ping.png}
			\end{center}
\end{minipage}\hfill
\begin{minipage}[t]{0.55\linewidth}
\begin{center}
			\fbox{Affichage des données}
			
			\bigskip
			
			\includegraphics[scale=0.38]{images/exo0-ping-capture.png}
			\end{center}
\end{minipage}

\item En mode simulation faire un clic droit sur la machine \texttt{192.168.1.1} et afficher les échanges de données. 

\begin{enumerate}
	\item Repérer les paquets de données émis par la machine \texttt{192.168.1.1} avec la commande \texttt{ping} et les réponses renvoyées par \texttt{192.168.1.2} avec la commande \texttt{pong}. 
	
	Les paquets sont structurés en couches et contiennent deux paires d'adresses MAC et IP.
	
	Reconstituer un  échange initié par une commande \texttt{ping}  entre les deux machines à l'aide de deux  flèches en plaçant les bonnes adresses MAC et IP à leurs extrémités.
\item Pourrait-on avoir une machine d'adresse MAC  \texttt{10:AG:FF:EH:6E:A0} ? On pourra rechercher sur le Web le format d'une adresse \href{https://fr.wikipedia.org/wiki/Adresse_MAC}{MAC}. 
\item Permuter les adresses IP des deux machines et tester la liaison en exécutant une nouvelle commande \texttt{ping}. Peut-on échanger les adresses MAC ? 
\item Pour une machine connectée à un réseau informatique, on  distingue adresse logique  et adresse physique. Quelles sont les bonnes correspondances avec les adresses IP et MAC ? 
\end{enumerate}
...
\section{Interconnexion de réseaux}

\begin{exerciceB}{Interconnexion de plusieurs machines avec un switch}

On veut désormais étendre notre réseau à 4 machines. 

Ouvrir le projet précédent \texttt{exercice1.fls} et l'enregistrer dans le même dossier sous le nom 
\texttt{exercice2.fls}.



\begin{enumerate}
    \item Supprimer le câble entre les machines \texttt{192.168.1.1}  et \texttt{192.168.1.2} et rajouter deux machines  de type portable d'adresses IP \texttt{192.168.1.3}  et \texttt{192.168.1.4}.
	\item Pour interconnecter plus de deux machines  on utilise un \textit{commutateur} ou \textit{switch} en Anglais. Créer un \textit{switch} relié aux quatre  machines.
	
Un \textit{switch} ressemble à une multiprise avec plusieurs ports Ethernet RJ45 auxquels sont reliés les machines du réseau local. Il établit une table de   correspondances entre adresse physique \href{https://fr.wikipedia.org/wiki/Adresse\_MAC}{MAC}  et ports. Lorsqu'il reçoit un paquet, il lit l'adresse MAC du destinataire et transmet le paquet sur le port correspondant. 

\item Tester la connexion entre les machines \texttt{192.168.1.1}  et les trois  autres avec la commande \texttt{ping}.
	 
	
\end{enumerate}

{\itshape
Dans cet exercice on a simulé  un \textbf{réseau local}, constitué de machines pouvant communiquer directement en pair à pair. Cela peut correspondre au réseau reliant différents appareils  dans un foyer avec la box jouant le rôle du \textit{switch} mais il nous manque l'interconnexion avec un réseau externe : l'Internet. De plus, par rapport à un réseau d'entreprise ou d'établissement scolaire, il nous manque une machine jouant le rôle de \textbf{serveur} pour distribuer des services aux autres machines qui seraient ses \textbf{clients}.  Chaque machine est identifiée par une adresse MAC et une adresse IP, pour un réseau local l'adresse MAC pourrait suffir. On va voir le rôle de l'adresse IP dans l'exercice suivant.   
}

\end{exerciceB}


\begin{exerciceB}{Interconnexion de réseaux avec un routeur}

On veut désormais interconnecter deux réseaux locaux avec un nouvel équipement appelé \textit{routeur}.


Ouvrir le projet précédent \texttt{exercice2.fls} et l'enregistrer dans le même dossier sous le nom 
\texttt{exercice3.fls}.

\begin{enumerate}
	\item En mode conception, ajouter un \textit{routeur} en sélectionnant 2 interfaces puis ajouter  un \textit{switch}, une machine de type portable et une autre  de type ordinateur. Paramétrer leurs interfaces réseaux avec les adresses IP  \texttt{192.168.2.1} pour le portable et \texttt{192.168.2.2} pour l'ordinateur. 
	


	\item Relier le routeur aux deux switchs et le deuxième switch au portable et à l'ordinateur du réseau ajouté.
	 \begin{center}
	\includegraphics[scale=0.4]{images/reseau-serveur-web.png}
\end{center}
	\item Faire un clic droit sur le \textit{routeur} puis configurer ses deux interfaces  : en assignant l'adresse IP \\ \texttt{192.168.1.254} à celle reliée au premier réseau et \texttt{192.168.2.254} à l'autre.
\begin{center}
	\includegraphics[scale=0.4]{images/parametrage-routeur-exo3.png}
\end{center}
\item En mode simulation, sur la machine \texttt{192.168.1.1} où la ligne de commandes est active, tester les connexions vers toutes les autres  machines avec la commande \texttt{ping}. 

\item Quelles sont les machines qui ne peuvent être atteintes ?

\item Pour atteindre ces machines depuis \texttt{192.168.1.1}, il faut configurer une passerelle sur cette machine \ie{} un équipement du même réseau local qui pourra relayer les paquets à destination de l'extérieur local.

Cet équipement d'interconnexion entre deux réseaux est le \textit{routeur}. 

Sa première interface \texttt{192.168.1.254} fait partie du même réseau que \texttt{192.168.1.1} et plus généralement que toutes les machines d'IP \texttt{192.168.1.X}  dont le masque de sous-réseau \texttt{255.255.255.0}  signifie que les trois premiers octets de leur adresse IP, \texttt{192.168.1}, constituent le préfixe caractéristique du réseau auquel elles appartiennent.

De même l'interface \texttt{192.168.2.254} du routeur appartient au même réseau que les machines \texttt{192.168.2.1} et \texttt{192.168.2.2}. 


On peut noter  qu'une adresse réseau (IP ou MAC) est liée à une \textbf{interface} et qu'un équipement peut posséder plusieurs interfaces. C'est le cas des routeurs qui doivent interconnecter différents réseaux (un réseau  local et Internet par exemple).  

\begin{enumerate}
	\item  Repasser en mode conception et configurer la passerelle \texttt{192.168.1.254} sur la machine \texttt{192.168.1.1}. Essayer de nouveau d'atteindre la machine \texttt{192.168.2.2} depuis la machine \texttt{192.168.2.1}. Test concluant ?

\begin{center}
	\includegraphics[scale=0.4]{images/parametrage-passerelle.png}
\end{center}

\item  Pour que \texttt{ping 192.168.2.2} fonctionne, il faut, comme on l'a déjà vu, que \texttt{192.168.2.2} renvoie un écho \texttt{pong} vers l'émetteur \texttt{192.168.1.1}.

Comme \texttt{192.168.1.1} est sur un autre réseau que \texttt{192.168.2.2}, il faut configurer la passerelle \texttt{192.168.2.254} (interface du routeur sur le même réseau que \texttt{192.168.2.2}) sur la machine 

\texttt{192.168.2.2}. On doit faire de même sur \texttt{192.168.2.1}.

\begin{center}
	\includegraphics[scale=0.4]{images/parametrage-passerelle2.png}
\end{center}

\item En mode simulation, vérifier que toutes les machines peuvent être désormais atteintes depuis \texttt{192.168.1.1} avec la commande \texttt{ping}.

\item 	Échanger les machines d'adresses IP \texttt{192.168.1.1} et \texttt{192.168.2.1}. L'adresse logique de chaque machine est ainsi modifiée. Est-ce le cas de son adresse physique ?

\item Comment l'adresse  IP permet-elle d'identifier les machines appartenant au même réseau local ?  

\end{enumerate}

\end{enumerate}

{\itshape Dans cet exercice, on a interconnecté deux réseaux locaux et on a vu deux fonctions du \href{https://fr.wikipedia.org/wiki/Internet_Protocol}{protocole IP} qui permet de faire communiquer des machines dans des réseaux différents :  \textbf{l'adressage} avec \href{https://fr.wikipedia.org/wiki/Adresse_IP}{l'adresse IP} et le \textbf{routage} avec les \textit{routeurs}. Cette fonctionnalité est l'objet du prochain exercice.}  

\end{exerciceB}
 

\begin{exerciceB}{Routage dans une interconnexion de réseaux}

Récupérer le fichier \texttt{exercice4\_ressources.fls} puis l'ouvrir avec \texttt{Filius}.

\begin{center}
\includegraphics[scale=0.3]{images/reseau-exo4.png}
\end{center}

\begin{enumerate}
\item Récupérer les adresses IP des machines \texttt{M14} et \texttt{M9}.

\item En mode simulation, faire un \texttt{ping} de la machine \texttt{M14} vers \texttt{M9} pour vérifier la connexion.
\item Exécuter une commande \texttt{traceroute} à partie de la machine \texttt{M14} pour afficher la route empruntée par les paquets de données pour atteindre la machine \texttt{M9}. Combien de routeurs ont été traversés ?  Marquer sur le schéma la route empruntée et les adresses IP des  interfaces des routeurs intermédiaires.

\begin{center}
\includegraphics[scale=0.5]{images/traceroute.png}
\end{center}


\item Supprimer le câble réseau (clic droit sur le câble) qui relie le routeur F au routeur E (simulation de panne) et refaire un \texttt{traceroute} de  M14 et M9. 

\bcattention{} Attendre un peu pour que les tables de routage des routeurs se mettent à jour.

%En cas de problème, récupérer et ouvrir le fichier \texttt{exercice4bis\_ressources.fls} pour faire le test.
...
{Dans cet exercice on a pu observer la fonction de \textbf{routage} du protocole \href{https://fr.wikipedia.org/wiki/Internet_Protocol}{protocole IP} et la souplesse qui permet aux routeurs de s'adapter à l'évolution du réseau en mettant à jour leurs tables de routage.
...
\section{Réseau avec serveur applicatif}


\begin{exerciceB}{Ajout d'un serveur de messagerie}


On veut désormais simuler un service réseau de courrier électronique avec échange client/serveur. 


Ouvrir le projet \texttt{exercice5.fls} et l'enregistrer dans le dossier du TP.

\begin{center}
\includegraphics[scale=0.5]{images/exo5-reseau.png}
\end{center}

\begin{enumerate}
	\item Passer en mode simulation et installer un serveur de messagerie sur la machine  \texttt{192.168.2.2}. Créer trois comptes dans le domaine \texttt{leparc.fr} :
	\begin{itemize}
		\item Pour Bob avec l'adresse \texttt{bob@leparc.fr}, le nom d'utilisateur \texttt{bob} et le mot de passe \texttt{bob}.
		\item Pour Alice avec l'adresse \texttt{alice@leparc.fr}, le nom d'utilisateur \texttt{alice} et le mot de passe \texttt{alice}.
		\item Pour Mallory avec l'adresse \texttt{mallory@leparc.fr}, le nom d'utilisateur \texttt{mallory} et le mot de passe \texttt{mallory}.
	\end{itemize}

Démarrer le serveur de messagerie. 

\begin{center}
\includegraphics[scale=0.5]{images/serveur-mail1.png}
	\end{center}


\item Afficher le bureau de  la machine de Bob d'IP \texttt{192.168.1.1}, installer un client de messagerie et configurer le compte. Quels sont les rôles des protocoles de service réseau \href{https://fr.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol}{SMTP} et \href{https://fr.wikipedia.org/wiki/Post_Office_Protocol}{POP3} ?

On voit apparaître de nouveaux paramètres d'adresse, les \textbf{ports}, qui identifient non pas des machines mais des applications de service réseau (25 pour \href{https://fr.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol}{SMTP} et 110 pour \href{https://fr.wikipedia.org/wiki/Post_Office_Protocol}{POP3}).

\begin{center}
\includegraphics[scale=0.5]{images/compte-mail1.png}
	\end{center}



	
\item Installer un client de messagerie et paramétrer le compte pour les machines d'Alice \texttt{192.168.1.3} et de Mallory \texttt{192.68.2.1}.

\item Depuis la machine de Bob, envoyer un message à Alice, puis afficher les données échangées sur l'interface \texttt{192.68.1.1}.

\begin{center}
\includegraphics[scale=0.5]{images/envoi-mail1.png}
	\end{center}

	\begin{enumerate}
		\item Identifier les quatre couches \textbf{Réseau}, \textbf{Internet}, \textbf{Transport} et \textbf{Application} qui constituent les différents entêtes encapsulant le paquet de données numéroté 20 dans l'image précédente.  Pour chaque entête, déterminer la paire d'adresses et ce qu'elles permettent d'identifier.
		
		\item Dans quelle couche s'exécute le protocole \texttt{IP} ? et le protocole \texttt{TCP} ?
		
		\item Lire l'article de Wikipedia sur l'ouverture d'une connexion TCP :  \url{https://fr.wikipedia.org/wiki/Three-way_handshake}. Repérer la \textit{poignée de main en trois temps} dans la figure précédente.
		
		\item  Le message émise par Bob est-il envoyé directement à la machine d'Alice ? L'envoi du message est-il effectué un en seul paquet ? Décrire les étapes du protocole \href{https://fr.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol}{SMTP} qui permet d'envoyer un message.
		
		\item  		On observe dans l'échange une alternance de messages successifs entre les machines d'interfaces \texttt{192.168.1.1} et  \texttt{192.168.2.2}? Un paquet avec un entête de couche application (protocole \href{https://fr.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol}{SMTP}) est toujours suivi d'un paquet sans entête application mais avec un entête TCP  portant   le commentaire \texttt{ACK}. En Anglais, \texttt{ACK} signifie aquittement, quel peut être le rôle de ces paquets ?
		
		\item Dans l'image ci-dessous, Repérer le paquet qui contient les données du message. Combien de paquets sont échangés pour fermer la connexion TCP ?
		
	
\begin{center}
\includegraphics[scale=0.5]{images/envoi-mail2.png}
	\end{center}


	\end{enumerate}
	
\item Depuis la machine d'Alice, récupérer le message de Bob depuis son client de messagerie et observer les données échangées. Quel protocole de la couche application est utilisé ? Quelles similarités peut-on observer dans les échanges de données par rapport à l'envoi d'un message ?

\begin{center}
\includegraphics[scale=0.5]{images/reception-mail1.png}
	\end{center}

\begin{center}
\includegraphics[scale=0.5]{images/reception-mail2.png}
	\end{center}
...
\item Envoyer un message d'Alice vers Bob en mettant en copie Mallory. Observer les échanges de données.
\end{enumerate}

{\itshape Dans cet exercice, on a pu observer plusieurs principes des échanges de données dans un réseau informatique : l'importance des protocoles et leur organisation en couches fonctionnelles et le découpage des données en paquets.}
